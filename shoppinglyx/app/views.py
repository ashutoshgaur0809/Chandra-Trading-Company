...
class ProductView(View):
    def get(self,request):
        ...
        groceryes=Product.objects.filter(category='G')
        furnitures=Product.objects.filter(category='F')
        if request.user.is_authenticated:
            totalitem=len(Cart.objects.filter(user=request.user))
        return render(request,'app/home.html',{'topwears':topwears,'bottomwears':bottomwears,'mobiles':mobiles,'laptops':laptops,'groceryes':groceryes,'furnitures':furnitures,'totalitem':totalitem})
...
def grocery(request,data=None):
    ...
    if data==None:
        groceryes=Product.objects.filter(category='G')
    elif data=='CTC' or data=='Raja':
        groceryes=Product.objects.filter(category='G').filter(brand=data)
    elif data=='below':
        groceryes=Product.objects.filter(category='G').filter(discounted_price__lt=500)
    elif data=='above':
        groceryes=Product.objects.filter(category='G').filter(discounted_price__gt=500)
    if request.user.is_authenticated:
        totalitem=len(Cart.objects.filter(user=request.user))
    return render(request, 'app/grocery.html',{'groceryes':groceryes,'totalitem':totalitem})

def furniture(request,data=None):
    totalitem=0
    if data==None:
        furnitures=Product.objects.filter(category='F')
    elif data=='Sofa' or data=='Chair':
        furnitures=Product.objects.filter(category='F').filter(brand=data)
    elif data=='below':
        furnitures=Product.objects.filter(category='F').filter(discounted_price__lt=500)
    elif data=='above':
        furnitures=Product.objects.filter(category='F').filter(discounted_price__gt=500)
    if request.user.is_authenticated:
        totalitem=len(Cart.objects.filter(user=request.user))
